function spline2D_comp_terrain()
  terrain = spline2Duniform();  % Zapisujemy wynik do zmiennej
  animate_terrain(terrain);  % Dodajemy animację
end

function terrain = spline2Duniform()
  image = imread("terrrain_map.jpg");  % Zmiana nazwy pliku
  ...
  terrain = combined_function;

  figure;
  surf(X, Y, combined_function, 'EdgeColor', 'none');
  colormap(parula);
  lighting gouraud;
  material dull;
  hold off;
...
% Nowa funkcja do animacji
function animate_terrain(terrain)
  fig = figure;
  % Prealokacja tablicy dla klatek
  frames = cell(1, ceil(360/10));
  frameIdx = 1;
  
  for angle = 1:10:360
      surf(terrain, 'EdgeColor', 'none');
      view(angle, 35);  % Obrót kamery
      colormap(parula);    % Dodajemy mapę kolorów do każdej klatki
      lighting gouraud;
      material dull;
      
      % Zapisz klatkę
      drawnow;  % Upewniamy się, że figura jest wyrenderowana
      frame = getframe(fig);
      frames{frameIdx} = frame2im(frame);
      frameIdx = frameIdx + 1;
      
      clf(fig);  % Czyścimy figurę przed następną klatką
  end
  ...
  close(fig);  % Zamykamy figurę po zakończeniu
end
