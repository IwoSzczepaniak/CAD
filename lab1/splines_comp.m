function splines_comp(precision, knot_vector, coefficients)
  ...
  % check if the size of coefficients vector matches the number of basis functions
  if length(coefficients) ~= nr
      disp("Size of coefficients vector does not match the number of basis functions")
      return
  end
  ...
  % initialize the result as a zero vector for the linear combination
  result = zeros(1, length(x));
  
  % calculate and plot the linear combination of basis functions
  for i = 1:nr
      spline_values = compute_splines(knot_vector, p, i, x);
      result = result + coefficients(i) * spline_values;
  end
  
  % plot the resulting linear combination of B-splines
  plot(x, result, 'LineWidth', 3);
  hold off
